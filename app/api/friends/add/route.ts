...
export async function POST(req: Request){
    try {
        ...
        if(!idToAdd){return new Response('This person does not exists', {status: 400})}

        if(!idToAdd){return new Response('This person does not exists', {status: 400})}
        const session = await getServerSession(authOptions);
        if(!session){return new Response('Unauthorized', {status: 401})}
        if(idToAdd === session.user.id){return new Response('You can not add yourself as friend', {status: 400})}
